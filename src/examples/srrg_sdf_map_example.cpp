...
float resolution = 0.05f, usable_range=5.0f, max_distance = 7.0f, min_distance = 0.4f;
...
int main(int argc, char ** argv) {
    ...
    while ((msg = reader.readMessage())) {
        ...
        if (image_msg) {
            ...
            zmax = zmax > z+usable_range ? zmax : z+usable_range;
            xmin = xmin < x-usable_range ? xmin : x-usable_range;
            ymin = ymin < y-usable_range ? ymin : y-usable_range;
            zmin = zmin < z-usable_range ? zmin : z-usable_range;
